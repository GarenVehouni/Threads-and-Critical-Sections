...
printf("The Performance evaluation using mutex method: "); 
...
int j;
	...
	for(j = 0; j <= 4; j++) {
		...
		simpson(tableThreadCount); //i ^ 2 is passed into simpson
		GET_TIME(finishTimeTable); //end time		 
		
		double elapsedTimeTable = finishTimeTable - startTimeTable;
		
		double speedupTable = elapsedOneThread / elapsedTimeTable; 
